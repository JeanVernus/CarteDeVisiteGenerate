...
class CarteDeVisite5 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      Nom: props.Nom,
  ...
  render() {
    ...
    return (
      <div className="cadreCarte5">
        <div><img src={Photo} alt="" width="250px" heigth="150px" /></div>
        <div><div><span>Nom: </span>{Nom}</div><div><span>Prénom: </span>{Prenom}</div><div><span>Poste: </span>{Poste}</div><br /></div>
...
export default connect(mapStateToProps)(CarteDeVisite5);